...
import numpy as np
from Archive.KG_Implementation.utils.Edge import Edge
from Archive.KG_Implementation.utils.Node import Node
from Archive.KG_Implementation.utils.PriorityQueue import PriorityQueue
        ...
        while(_current_node and _current_distance < distance):
            ...
            for connected_node_name in self._edges[_current_node.name]:
                ...
                if self._edges[_current_node.name][connected_node_name].get_weight() <= 0:
                    temp_score = _scores[_current_node.name][_current_distance] - log(0.1) - _current_distance * log(self._alpha)
                else:
            ...
            if not _recomendees:
                ...
                while(left_pointer != right_pointer):
                    ...
                    if _data_tuples[middle_pointer][0][1] <= data_tuple[0][1]:
                        right_pointer = int((left_pointer + middle_pointer) / 2)
                        continue
                    else:
                        left_pointer = int((right_pointer + middle_pointer) / 2)
                        continue
    ...
    def load_from_csv(file_descriptor, name_file):
        ...
        for i in range(len(list_of_dict_edges)):
            if list_of_dict_edges[i]['From Name'] in name_list and list_of_dict_edges[i]['To Name'] in name_list:
                ...
                start_node=list_of_dict_edges[i]['From Name'], end_node=list_of_dict_edges[i]['To Name'], weight=1, 
